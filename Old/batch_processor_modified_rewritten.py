...
class BatchProcessor:
    ...
    def _display_main_menu(self):
        ...
        print("10. Edit Existing Addon Entries")
        print("11. Exit")
        print("="*60)
    ...
    def interactive_entry_editor(self, existing_addon: Dict[str, Any], official_entry: Optional[Dict[str, Any]] = None) -> Optional[Dict[str, Any]]:
        """
        Interactive UI for editing existing addon entries
        
        Args:
            existing_addon: The existing addon entry to edit
            official_entry: Optional official theme entry for reference
            
        Returns:
            Dict or None: Updated addon entry or None if skipped
        """
        repo = existing_addon.get("repo")
        name = official_entry.get("name", "Unknown") if official_entry else "Unknown"
        author = official_entry.get("author", "Unknown") if official_entry else "Unknown"
        modes = official_entry.get("modes", []) if official_entry else []
        
        print(f"\nEditing addon entry for: {name} by {author}")
        print(f"Repository: {repo}")
        print("-" * 50)

        # Offer to open the GitHub page
        print("\nWould you like to open this GitHub repository in your browser? (y/n, default n): ")
        response = input().strip().lower()
        if response == 'y':
            open_github_repo(repo)
        
        # Start with copy of existing
        addon_entry = existing_addon.copy()
        
        # Screenshot main
        current_main = existing_addon.get("screenshot-main", "")
        print(f"\n1. Main screenshot (current: '{current_main}')")
        print("   Options: enter to keep, or provide new URL/filename")
        main_screenshot = input("   Main screenshot: ").strip()
        if main_screenshot.lower() == 'exit':
            return None
        if main_screenshot:
            addon_entry["screenshot-main"] = main_screenshot
        
        # Additional screenshots
        current_side = existing_addon.get("screenshots-side", [])
        print(f"\n2. Additional screenshots (current: {', '.join(current_side)})")
        print("   Enter new comma-separated URLs, or enter to keep current")
        side_input = input("   Additional screenshots: ").strip()
        if side_input.lower() == 'exit':
            return None
        if side_input:
            screenshots_side = [u.strip() for u in side_input.split(',') if u.strip()]
            addon_entry["screenshots-side"] = screenshots_side
        
        # Tags
        current_tags = existing_addon.get("tags", [])
        print(f"\n3. Tags (current: {', '.join(current_tags)})")
        print("   Enter new comma-separated tags, or enter to keep")
        tags_input = input("   Tags: ").strip()
        if tags_input.lower() == 'exit':
            return None
        if tags_input:
            ...
            tags = current_tags
        # Auto-assign dark/light/dark_and_light based on modes if applicable
        default_tags = []
        if 'dark' in modes:
            default_tags.append('dark')
        if 'light' in modes:
        ...
        # Show preview
        print("\n" + "="*50)
        print("UPDATED ADDON ENTRY PREVIEW:")
        print(json.dumps(addon_entry, indent=2))
        print("="*50)
        
        return addon_entry
    
    def _menu_edit_addon_entries(self):
        """Menu option for editing existing addon entries"""
        print("\n" + "-"*50)
        print("EDIT EXISTING ADDON ENTRIES")
        print("-"*50)
        
        official_data = self._load_official_data()
        official_lookup = {entry.get('repo'): entry for entry in official_data if entry.get('repo')}
        
        addon_data = self._load_addon_data()
        
        if not addon_data:
            print("No existing addon entries found.")
            return
        
        print(f"Found {len(addon_data)} addon entries:")
        for i, addon in enumerate(addon_data, 1):
            repo = addon.get('repo', 'unknown')
            official = official_lookup.get(repo, {})
            name = official.get('name', 'Unknown')
            author = official.get('author', 'Unknown')
            print(f"{i:3}. {name} by {author} ({repo})")
        
        choice = input("\nSelect number to edit (0 to cancel): ").strip()
        if choice == '0':
            return
        
        try:
            index = int(choice) - 1
            if not 0 <= index < len(addon_data):
                print("Invalid selection.")
                return
        except ValueError:
            print("Invalid input.")
        ...
        existing_addon = addon_data[index]
        repo = existing_addon.get('repo')
        official = official_lookup.get(repo)
        
        new_addon = self.interactive_entry_editor(existing_addon, official)
        
        if new_addon:
            addon_data[index] = new_addon
            if self._save_addon_data(addon_data):
                print("✅ Addon entry updated and saved!")
            else:
                print("❌ Failed to save updated entry.")
        else:
