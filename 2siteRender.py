...
class StandaloneThemeRenderer:
    ...
    def prepare_themes_for_rendering(self, official_data: List[Dict], addon_data: List[Dict]) -> tuple[List[Dict], List[str]]:
        ...
        for official_theme in official_data:
            ...
            if addon_theme:
                # Get screenshot URLs and convert to GitHub raw URLs if needed
                main_screenshot = addon_theme.get("screenshot-main", official_theme.get("screenshot", ""))
                main_screenshot_url = self._convert_to_github_url(main_screenshot, repo)
                
                # Process additional screenshots
                additional_screenshots = addon_theme.get("screenshots-side", [])
                additional_image_urls = [
                    self._convert_to_github_url(screenshot, repo) 
    ...
    def _convert_to_github_url(self, screenshot_path: str, repo: str) -> str:
        """
        Convert screenshot path to proper GitHub raw URL if it's a local filename
        
        Args:
            screenshot_path: Could be filename, relative path, or full URL
            repo: GitHub repo in format "user/repository"
            
        Returns:
            Full GitHub raw URL to the screenshot
        """
        ...
        # If it's a local filename or relative path, convert to GitHub raw URL
        # Assume screenshots are in the root of the repository
        # Format: https://raw.githubusercontent.com/user/repo/HEAD/filename.png
        ...
        return f"https://raw.githubusercontent.com/{repo}/HEAD/{screenshot_path}"

    def render_all_themes(self, force: bool = False, debug: bool = False) -> Dict[str, Any]:
        """
        Render all themes with complete data
        
        Args:
            force: If True, skip confirmation prompt
            debug: If True, show detailed data for first theme
        ...
        # Debug output for first theme
        if debug and complete_themes:
            print(f"\nDEBUG: First theme data structure:")
            import json
            print(json.dumps(complete_themes[0], indent=2))
    ...
    def interactive_menu(self):
        ...
        while True:
            ...
            print("6. Debug Mode (Render with data inspection)")
            print("7. Exit")
            print("="*60)
            ...
            try:
                if choice == '1':
                    ...
                    # Debug mode for first theme
                    result = self.render_all_themes(force=True, debug=True)
                    self._print_result(result)
                elif choice == '7':
                    print("Goodbye!")
                    break
